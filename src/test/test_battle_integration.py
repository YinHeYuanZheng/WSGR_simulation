...
from src.utils.loadConfig import load_config, load_yaml, load_xml
...
class BattleIntegrationTest(unittest.TestCase):
    ...
    def test_map_battle(self):
        """真实数据库和战斗配置应能完成一场完整地图战斗。"""
        database_file = os.path.join(
            PROJECT_DIR, 'depend', 'ship', 'database.xlsx'
        )
        config_file = os.path.join(
            PROJECT_DIR, 'config', 'config_map_test.xml'
        )
        map_dir = os.path.join(PROJECT_DIR, 'depend', 'map')
