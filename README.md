...
- 不读取 `self.value` 的 Python 技能可以省略参数，例如 `@632`。
- 需要参数的技能在冒号后填写，多个参数使用逗号分隔。所需参数数量由对应
  `esid` 代码决定，例如 `@013` 至少需要一个参数，`@018` 至少需要两个。
- 多个 Python 定向或其他配置项使用分号并列。
- Python 定向可以被包装器限制，但不能参与普通词条的大括号合并组。
